...
def download_game(game, process, path_to_raw, path_to_final):
    ...
    if process == True:
        try:
            processed_data = sdv.nba.nba_pbp_disk(game_id=game, path_to_json=path_to_raw)

            result = sdv.nba.helper_nba_pbp(game_id=game, pbp_txt=processed_data)
...
def main():
    ...
    for year in years_arr:
        ...
        if args.rescrape == False:
            schedule_in_repo = pd.read_parquet(
                "https://raw.githubusercontent.com/sportsdataverse/hoopR-nba-data/blob/main/nba/nba_games_in_data_repo.parquet",
                engine="auto",
                columns=None,
            )
