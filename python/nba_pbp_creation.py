...
def nba_team_box_score_extract(js, result):
    team_box_score = pd.json_normalize(
        result.get("boxscore", {}).get("teams", []),
...
def nba_player_box_score_extract(js, result):
    ...
    for team in num_teams:
        ...
        team_info = pd.json_normalize(data=result.get("boxscore").get("players", [])[team].get("team"), errors="ignore")
            ...
            if len(aths_column[i]) > 0:
                for k in range(len(aths_column[i])):
                    try:
                        ...
                        athlete_df = pd.json_normalize(athlete.get("athlete", {}), errors="ignore")
...
def main():
    ...
    for year in years_arr:
        ...
        season_schedule = pd.read_parquet(
            f"{path_to_schedules}/parquet/nba_schedule_{year}.parquet", engine="auto", columns=None
        )
        ...
        json_files = [
            pos_json.replace(".json", "") for pos_json in os.listdir(path_to_final_json) if pos_json.endswith(".json")
        ]
...
def str2bool(value):
    ...
    ``-p false`` silently ran the compilation anyway. Mirrors the
    wehoop-wbb-raw house converter -- unrecognised text parses as False.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Run play-by-play compilation for all games in the schedule period",
    )
    parser.add_argument(
        "--team_box",
        "-t",
        type=str2bool,
        default=True,
        help="Run team boxscore compilation for all games in the schedule period",
    )
    parser.add_argument(
        "--player_box",
        "-b",
        type=str2bool,
        default=True,
        help="Run player boxscore compilation for all games in the schedule period",
    )
