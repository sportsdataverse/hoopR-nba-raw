"""Scrape ESPN NBA team season stats per (season, team_id).

Output: ``nba/team_stats/json/{season}/{team_id}.json`` -- the raw ESPN
``/teams/{team_id}/statistics?season=YYYY`` response. The downstream R
parser in ``hoopR-nba-data`` (``espn_nba_06_team_season_stats_creation.R``)
reads these JSONs to build the per-season tidy team-stats frame.

Unlike the per-athlete stats endpoint, this team endpoint DOES honor the
``season`` query param (the payload carries ``requestedSeason`` and the
...
Requirements:
    ...
    sportsdataverse-py (``sportsdataverse/wbb/wbb_team_stats.py``), invoked
    with ``league="nba"``.
"""
...
logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_nba_raw_team_stats_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "nba/team_stats/json"
PATH_TO_SCHEDULES = "nba/schedules/parquet"
DEFAULT_THREADS = 4


def fetch_team_ids_for_season(season: int) -> list[int]:
    """Unique team ids for the season, from the schedule parquet
    (home_id / away_id)."""
...
def download_team_stats(
    ...
    try:
        raw = _espn_basketball_team_stats(
...
def scrape_season(season: int, cores: int, rerun_existing: bool) -> None:
    ...
    t1 = time.time()
    logger.info(
        f"{(t1 - t0) / 60:.2f} minutes to download {len(team_ids)} "
        f"team-stat payloads for {season}."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Start year of NBA team-stats scrape (YYYY)",
    )
    parser.add_argument(
        "--end_year", "--end-year", "-e", dest="end_year", type=int,
        help="End year of NBA team-stats scrape (YYYY)",
    )
    parser.add_argument(
